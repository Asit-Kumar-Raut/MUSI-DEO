...
const MusicHome = () => {
  ...
  const checkConnection = async () => {
    try {
      const BACKEND_ROOT = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
        ? 'http://localhost:5001'
        : 'https://musi-deo.vercel.app';
      const res = await axios.get(BACKEND_ROOT);
